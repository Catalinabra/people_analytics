view: pop_support {
  derived_table: {
    sql: select periods_ago
    from (
    select 0 as periods_ago {% if periods_ago._in_query%}
    {% for i in (1..52)%} union all select {{i}}{%endfor%}{%endif%} )
    possible_periods where {%condition periods_ago_to_include%}periods_ago{%endcondition%} {% if periods_ago_to_include._is_filtered == false%}and periods_ago <=1{%endif%};; }
  ...
  parameter: period_size {
    ...
    allowed_value: {value:"Default"
      label:"Default Based on Selection"}
  ...
  dimension: pop_sql_years_using_now {
    type: date_raw
    expression: add_years(${periods_ago},${now_sql});;
  }
  dimension: pop_sql_months_using_now {
    type: date_raw
    expression: add_months(${periods_ago},${now_sql});;
  }
  dimension: pop_sql_days_using_now {
    type: date_raw
    expression: add_days(${periods_ago},${now_sql});;
  }
  dimension: period_label_sql {
    hidden:yes
    expression: if(${pop_support.periods_ago}=0," Current" , concat( ${pop_support.periods_ago}," REPLACE_WITH_PERIOD" ,if(${pop_support.periods_ago}>1,"s","") ," Prior" ) );;
  }
